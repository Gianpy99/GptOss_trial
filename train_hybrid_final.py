...
# Parametri OTTIMIZZATI per hybrid training (F1 + Tolkien)
MODEL_NAME = "google/gemma-3-4b-it"
PROJECT_NAME = "hybrid_expert"
EPOCHS = 2           # 2 epochs per training completo
BATCH_SIZE = 2
LIMIT = 100          # 100 esempi combinati
MAX_LENGTH = 512     # Lunghezza completa per contenuti più lunghi

print(f"⚙️ Parametri Hybrid Expert:")
...
# Load combined dataset (F1 + Tolkien)
print(f"📥 Loading combined dataset (F1 + Tolkien)...")
import json
with open("combined_f1_tolkien_data.json", 'r', encoding='utf-8') as f:
    dataset = json.load(f)

# Take only LIMIT examples
if len(dataset) > LIMIT:
    dataset = dataset[:LIMIT]

print(f"✓ Dataset loaded: {len(dataset)} examples")
print(f"  - Sample: {dataset[0].get('instruction', '')[:60]}...")
print()

# Prepare training data - formato instruction/output
